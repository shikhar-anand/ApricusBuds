...
class Placeholders {
	...
	public function __construct( Actions $wp_actions, Image $wp_image, MediaLibrary $media_library ) {
		...
		// Views / WPA content
		$wp_actions->add_filter( 'wpv_filter_wpv_view_shortcode_output', array( $this, 'replace_placeholders' ), 10, 1 );
		$wp_actions->add_filter( 'toolset_the_content_wpa', array( $this, 'replace_placeholders' ), PHP_INT_MAX - 1, 1 );
		$wp_actions->add_filter( 'wpv_filter_wpv_widget_output', array( $this, 'replace_placeholders' ), PHP_INT_MAX - 1, 1 );
	}
