...
class CRED_Field_Command_Relationships extends CRED_Field_Command_Base {
	...
	public function execute() {
		...
		$max_results = ( isset( $this->filtered_attributes['max_results'] ) )
			? $this->filtered_attributes['max_results'] : null;
		$use_select2 = ( isset( $this->filtered_attributes['use_select2'] ) )
			? $this->filtered_attributes['use_select2'] : null;
		...
		$field['order_by'] = ( isset( $this->filtered_attributes['order'] ) ) ? $this->filtered_attributes['order']
			: 'ID';
		$field['order'] = ( isset( $this->filtered_attributes['ordering'] ) ) ? $this->filtered_attributes['ordering']
			: 'DESC';
		...
		/*
		 * If it is editing a RFG item
		 */

		if ( $this->form->get_form_type() === 'edit' ) {
			$form_fields = $this->form->getFields();
			if ( isset( $form_fields[ 'form_settings' ] ) && isset( $form_fields['form_settings']->post ) ) {
		...
		$current_value = isset( $default_option )
