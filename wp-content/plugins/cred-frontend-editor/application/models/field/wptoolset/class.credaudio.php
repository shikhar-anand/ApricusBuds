...
class WPToolset_Field_Credaudio extends CRED_Abstract_WPToolset_Field_Credfile {
	...
	protected function get_extension_error() {
		...
		return $media_model->get_upload_validation_error_message( 'audio' );
	}
	...
	protected function get_select_label() {
		...
		if ( empty( $select_label ) ) {
			/* translators: Default label for a button to select an audio file field value */
			$select_label = __( 'Upload or select audio', 'wp-cred' );
		} else {
			// Translate the label
		}
		return $select_label;
	...
	protected function get_edit_label() {
		...
		if ( empty( $edit_label ) ) {
			/* translators: Default label for a button to edit or modify an audio file field value */
			$edit_label = __( 'Replace audio', 'wp-cred' );
		} else {
			// Translate the label
		}
		return $edit_label;
