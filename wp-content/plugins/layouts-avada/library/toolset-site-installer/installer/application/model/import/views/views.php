...
class TT_Import_Views_Views extends TT_Import_Items_Group_Abstract
{
    ...
    {
        return __('Views', 'toolset-themes');
    }
    ...
    {
        if ( ! class_exists('WPV_Embedded_View_Item_Provider_Decorator')) {
            return false;
        }

        $collector = new WPV_Embedded_View_Item_Provider_Decorator();
    ...
    protected function fetchItemsToImport()
    {
        ...
        foreach ( $import as $item) {
            if ( array_key_exists( 'meta', $item ) && array_key_exists('_wpv_settings', $item['meta'] )
                 && $item['meta']['_wpv_settings']['view-query-mode'] == 'normal'
            ) {
                $items[] = (object)$item;
            }
        }
