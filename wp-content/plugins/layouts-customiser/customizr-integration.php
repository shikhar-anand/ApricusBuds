<?php
/*
Plugin Name: Toolset Customizr Integration
Plugin URI: http://wp-types.com/
Description: Layouts Integration for Customizr
...
if( !defined('TOOLSET_INTEGRATION_PLUGIN_THEME_NAME') ){
    define('TOOLSET_INTEGRATION_PLUGIN_THEME_NAME','Customizr');
}

class WPDDL_Customizr_Integration_Loader {
		...
		/** @noinspection PhpUnusedParameterInspection */ $layouts_version, $integration_support_version )
	{
		...
		if( version_compare( $layouts_version, '1.4.5' ) !== -1 && $supported_integration_api_version == $integration_support_version ) {
			...
			$loader = WPDDL_Integration_Customizr::get_instance();
...
WPDDL_Customizr_Integration_Loader::getInstance();
