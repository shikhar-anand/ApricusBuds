...
class TT_Import_Wordpress extends TT_Import_Abstract {
	...
	protected function getPathImportFile() {
		if ( ! $this->is_path_default ) {
			// user set path, let's keep it
	...
	private function adjustImportFileToUseThemeUploads() {
		...
		$path_modified_import_file = trailingslashit( $wp_upload_dir['basedir'] )
		                             . 'toolset-installer-wordpress-'
		                             . TT_THEME_SLUG . '-' . TT_THEME_VERSION . '.xml';

		if ( file_exists( $path_modified_import_file ) ) {
			// file with adjusted paths already created
			$this->setPathImportFile( $path_modified_import_file );
			return $path_modified_import_file;
		}

		if ( ! wp_mkdir_p( dirname( $path_modified_import_file ) )
		     || ! file_put_contents( $path_modified_import_file, 'Write test' )
		) {
		...
		if ( file_put_contents( $path_modified_import_file, $string ) ) {
			$this->setPathImportFile( $path_modified_import_file );
		};
	}
