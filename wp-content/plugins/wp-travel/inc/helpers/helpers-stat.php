...
function wptravel_get_booking_data() {
	...
	if ( ! isset( $submission_request['chart_type'] ) || ( isset( $submission_request['chart_type'] ) && 'booking' === $submission_request['chart_type'] ) ) {
		...
		$results           = get_site_transient( '_transient_wt_booking_stat_data' );
		$initial_transient = $results;
		if ( ( ! $initial_load ) || ( $initial_load && ! $results ) ) {
			$results = $wpdb->get_results(
				$wpdb->prepare(
					...
					where post_type='itinerary-booking' AND post_status='publish' %s group by %s YEAR(post_date), MONTH(post_date), DAY(post_date) %s",
					$where,
					$groupby,
					$limit
				)
		...
		$results = $wpdb->get_results(
			$wpdb->prepare(
				"Select count( BOOKING.ID ) as wt_total, YEAR( payment_date ) as wt_year, Month( payment_date ) as wt_month, DAY( payment_date ) as wt_day, sum( AMT.payment_amount ) as payment_amount from {$wpdb->posts} BOOKING
				join (
				...
				where post_status='publish' and post_type = 'itinerary-booking' %s
				group by YEAR( payment_date ), Month( payment_date ), DAY( payment_date ) order by YEAR( payment_date ), Month( payment_date ), DAY( payment_date ) asc %s",
				$where,
				$limit
			)
		);
	...
	$results           = get_site_transient( '_transient_wt_booking_top_country' );
	$initial_transient = $results;
	if ( ( ! $initial_load ) || ( $initial_load && ! $results ) ) {
		...
		$results       = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT count(ID) as wt_total, country
				from (
				...
				) Booking where post_type='itinerary-booking' AND post_status='publish' %s  group by country order by wt_total desc",
				$where
			)
		);
		// Set initial load transient for stat data.
	...
	$results           = get_site_transient( '_transient_wt_booking_top_itinerary' );
	$initial_transient = $results;
	if ( ( ! $initial_load ) || ( $initial_load && ! $results ) ) {

		$results = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT count(ID) as wt_total, itinerary_id
				from (
					Select P.ID, P.post_date, P.post_type, P.post_status, C.country, I.itinerary_id from  {$wpdb->posts} P
					join ( Select distinct( post_id ), meta_value as country from {$wpdb->postmeta} WHERE meta_key = 'wp_travel_country' and meta_value != '' ) C on P.ID = C.post_id
					join ( Select distinct( post_id ), meta_value as itinerary_id from {$wpdb->postmeta} WHERE meta_key = 'wp_travel_post_id' ) I on P.ID = I.post_id
				...
				where post_type='itinerary-booking' AND post_status='publish' %s  group by itinerary_id order by wt_total desc",
				$where
			)
		);
		// set initial load transient for stat data.
	...
	if ( isset( $submission_request['compare_stat'] ) && 'yes' === $submission_request['compare_stat'] ) {
		...
		$results       = $wpdb->get_results(
			$wpdb->prepare(
				...
				where post_type='itinerary-booking' AND post_status='publish' %s  group by country order by wt_total desc",
				$where
			)
		);
	...
	$data          = apply_filters( 'wp_travel_stat_data', $data, $submission_request ); // @phpcs:ignore
	$data          = apply_filters( 'wptravel_stat_data', $data, $submission_request );
	...
	if ( isset( $submission_request['compare_stat'] ) && 'yes' === $submission_request['compare_stat'] ) {
		...
		// Query for total 2 in compare stat.
			$results = $wpdb->get_results(
				$wpdb->prepare(
					"Select count( BOOKING.ID ) as no_of_payment, YEAR( payment_date ) as payment_year, Month( payment_date ) as payment_month, DAY( payment_date ) as payment_day, sum( AMT.payment_amount ) as payment_amount from {$wpdb->posts} BOOKING
					join (
						Select distinct( PaymentMeta.post_id ), meta_value as payment_id, PaymentPost.post_date as payment_date from {$wpdb->posts} PaymentPost
						join {$wpdb->postmeta} PaymentMeta on PaymentMeta.meta_value = PaymentPost.ID
						WHERE PaymentMeta.meta_key = 'wp_travel_payment_id'
...
function wptravel_get_booking_status() {
	...
	$status = apply_filters( 'wp_travel_booking_status_list', $status ); // @phpcs:ignore
	return apply_filters( 'wptravel_booking_status_list', $status );
}

/**
 * Function to create stats data.
 *
 * @param array $stat_datas Stats data.
 * @param bool  $show_empty Show empty or not.
 */
