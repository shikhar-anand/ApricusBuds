...
final class CRED_Commerce {
    ...
    public static function _init_() {
        ...
        if (is_admin()) {
			...
			// Add extra options on Notification placeholders.
			add_filter( 'cred_admin_notification_placeholders', array( __CLASS__, 'register_notification_placeholders' ) );
    ...
    public static function cred_delete_draft_users() {
        $confirm = __("Are you sure ?", 'wp-cred-pay');
        $done = __("All draft users have been deleted succesfully", 'wp-cred-pay');
        $error = __("Error in deleting draft users", 'wp-cred-pay');
        ...
        . __("Delete all Users Draft", 'wp-cred-pay') . '</a></label>';
    }
    ...
    public static function cred_commerce_display_notice() {
        ?>
        <div class="error">
            <p><?php _e('Toolset Forms Commerce plugin needs <a href="https://toolset.com/home/toolset-components/?utm_source=plugin&utm_medium=gui&utm_campaign=formscommerce#forms" target="_blank"><strong>Toolset Forms</strong></a> to be installed and activated.', 'wp-cred-pay'); ?></p>
        </div>
        <?php
    }
    ...
    public static function addMetaBoxes($form) {
        global $pagenow;
        // commerce meta box
        add_meta_box('credcommercediv', __('Toolset Forms Commerce', 'wp-cred-pay'), array(__CLASS__, 'addCommerceMetaBox'), null, 'normal', 'high', array());
    }
    ...
    public static function addUserMetaBoxes($form) {
        global $pagenow;
        // commerce meta box
        add_meta_box('credcommercediv', __('Toolset Forms Commerce', 'wp-cred-pay'), array(__CLASS__, 'addUserCommerceMetaBox'), null, 'normal', 'high', array());
    }
	...
	public static function register_notification_placeholders( $placeholders ) {
		// Product data.
		$placeholders['%%PRODUCT_ID%%'] = array(
			'label' => __('Product ID', 'wp-cred-pay'),
			'placeholder' => '%%PRODUCT_ID%%',
		);
		$placeholders['%%PRODUCT_NAME%%'] = array(
			'label' => __('Product Name', 'wp-cred-pay'),
			'placeholder' => '%%PRODUCT_NAME%%',
		);
		$placeholders['%%PRODUCT_PRICE%%'] = array(
			'label' => __('Product Price', 'wp-cred-pay'),
			'placeholder' => '%%PRODUCT_PRICE%%',
		);
		// Customer data.
		$placeholders['%%CUSTOMER_ID%%'] = array(
			'label' => __('Customer ID', 'wp-cred-pay'),
			'placeholder' => '%%CUSTOMER_ID%%',
		);
		$placeholders['%%CUSTOMER_EMAIL%%'] = array(
			'label' => __('Customer Email', 'wp-cred-pay'),
			'placeholder' => '%%CUSTOMER_EMAIL%%',
		);
		$placeholders['%%CUSTOMER_DISPLAYNAME%%'] = array(
			'label' => __('Customer Display Name', 'wp-cred-pay'),
			'placeholder' => '%%CUSTOMER_DISPLAYNAME%%',
		);
		$placeholders['%%CUSTOMER_FIRSTNAME%%'] = array(
			'label' => __('Customer First Name', 'wp-cred-pay'),
			'placeholder' => '%%CUSTOMER_FIRSTNAME%%',
		);
		$placeholders['%%CUSTOMER_LASTNAME%%'] = array(
			'label' => __('Customer Last Name', 'wp-cred-pay'),
    ...
    public static function addCommerceExtraNotificationRecipients($form, $data, $notification) {
        ...
        if (!$notification || empty($notification)) {
            ...
            <p>
                ...
                <span data-cred-bind="{ action: 'show', condition: '_cred[notification][notifications][<?php echo $data[0]; ?>][to][type] has customer' }">
                    <select style="width:60px" name="_cred[notification][notifications][<?php echo $data[0]; ?>][to][customer][to_type]">
                        <option value="to"><?php _e('To:', 'wp-cred-pay'); ?></option>
                        <option value="cc"><?php _e('Cc:', 'wp-cred-pay'); ?></option>
                        <option value="bcc"><?php _e('Bcc:', 'wp-cred-pay'); ?></option>
                    </select><br />
            ...
            <p>
                ...
                <span data-cred-bind="{ action: 'show', condition: '_cred[notification][notifications][<?php echo $data[0]; ?>][to][type] has customer' }">
                    <select style="width:60px" name="_cred[notification][notifications][<?php echo $data[0]; ?>][to][customer][to_type]">
                        <option value="to" <?php if ('to' == $to_type) echo 'selected="selected"'; ?>><?php _e('To:', 'wp-cred-pay'); ?></option>
                        <option value="cc" <?php if ('cc' == $to_type) echo 'selected="selected"'; ?>><?php _e('Cc:', 'wp-cred-pay'); ?></option>
                        <option value="bcc" <?php if ('bcc' == $to_type) echo 'selected="selected"'; ?>><?php _e('Bcc:', 'wp-cred-pay'); ?></option>
                    </select><br />
                    <!--<input value="to" style="width:60px" name="_cred[notification][notifications][<?php echo $data[0]; ?>][to][customer][to_type]" type="hidden">-->
                </span>
            </p>
            ...
            <p>
                <label>
                    <input data-cred-bind="{
                           ...
                           }" type="radio" class="js-cred-notification-trigger" name="<?php echo $data[1]; ?>" value="order_created" />
            ...
            <p>
                <label>
                    <input data-cred-bind="{
                           ...
                           }" type="radio" class="js-cred-notification-trigger" name="<?php echo $data[1]; ?>" value="order_created" <?php if ($data[2] == 'order_created') echo 'checked="checked"'; ?> />
                    <span><?php _e('When submitting the form with payment details', 'wp-cred-pay'); ?></span>
                </label>
            </p>
            <?php
            ...
            <p>
                <label>
                    <input data-cred-bind="{
                           ...
                           }" type="radio" class="js-cred-notification-trigger" name="<?php echo $data[1]; ?>" value="payment_complete" />
            ...
            <p>
                <label>
                    <input data-cred-bind="{
                           ...
                           }" type="radio" class="js-cred-notification-trigger" name="<?php echo $data[1]; ?>" value="payment_complete" <?php if ($data[2] == 'payment_complete') echo 'checked="checked"'; ?> />
                    <span><?php _e('When payment is complete', 'wp-cred-pay'); ?></span>
                </label>
            </p>
            <?php
            ...
            <p>
                <label>
                    <input data-cred-bind="{
                           ...
                           }" type="radio" class="js-cred-notification-trigger" name="<?php echo $data[1]; ?>" value="order_modified" />
            ...
            <p>
                <label>
                    <input data-cred-bind="{
                           ...
                           }" type="radio" class="js-cred-notification-trigger" name="<?php echo $data[1]; ?>" value="order_modified" <?php if ($data[2] == 'order_modified') echo 'checked="checked"'; ?> />
                    <span><?php _e('When the purchase status changes to:', 'wp-cred-pay'); ?></span>
                </label>
                <span data-cred-bind="{ action: 'show', condition: '<?php echo $data[1]; ?>=order_modified' }">
                    <select class="cred_commerce_when_order_status_changes" name="_cred[notification][notifications][<?php echo $data[0]; ?>][event][order_status]">
