...
/**
 * Class CRED_Commerce_OnOrderCreated_Event
 *
 * @since 1.7
 */
class CRED_Commerce_Event_OnOrderCreated extends CRED_Commerce_Event_Base {
			...
			foreach ( $cred_forms_meta as $form_id => $cred_form_meta_object ) {
				...
				if ( $is_commerce ) {
					...
					if ( class_exists( $notification_class ) ) {
						...
						$notification_handler->on_order_created_event();
					}
				}
			}

			// HOOKS API
			do_action( 'cred_commerce_after_order_created', $this->data_order );
		}
	}
