...
/**
 * Class CRED_Commerce_OnHold_Event
 *
 * @since 1.7
 */
class CRED_Commerce_Event_OnHold extends CRED_Commerce_Event_Base {
		...
		if ( ! empty( $cred_forms_meta ) ) {
			foreach ( $cred_forms_meta as $form_id => $cred_form_meta_object ) {
				...
				if ( $is_commerce ) {
					...
					$notification_handler->on_hold_event();
				}
			}

			// HOOKS API
			do_action( 'cred_commerce_after_on_hold_event', $this->data_order );
		}
	}
}