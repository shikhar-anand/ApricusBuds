...
class Toolset_Framework_Installer_Configure extends Toolset_Framework_Installer_Install_Step {
	...
	function configure_site() {
		...
		$this->configure_wpml( $frameworkinstaller );
	 ...
	 *
	 * @param Toolset_Framework_Installer $frameworkinstaller Framework Installer instance.
	 */
	protected function configure_wpml( $frameworkinstaller ) {
		...
		if ( isset( $_POST['wpml'] ) && 'no-wpml' === $_POST['wpml'] && $is_wpml_site ) {
			...
			$status = $frameworkinstaller->download_file( $download_url, $target_path );
