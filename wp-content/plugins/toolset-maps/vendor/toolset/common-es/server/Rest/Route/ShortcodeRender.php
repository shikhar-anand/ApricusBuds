...
class ShortcodeRender extends ARoute {
	...
	public function callback( \WP_REST_Request $rest_request ) {
		...
		foreach( $params as $cachehash => $param ) {
			$hack = $hackFactory->get_hack( $param['current_post_id'], $param['shortcode'] );
			$hack->do_hack();
			if( isset( $param[ 'with_meta'] ) && $param[ 'with_meta' ] ) {
				$shortcode_content = $this->shortcode_render_with_meta->get_response_data( $param['current_post_id'], $param['shortcode'] );
