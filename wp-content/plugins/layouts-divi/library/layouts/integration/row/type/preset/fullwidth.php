...
abstract class WPDDL_Integration_Row_Type_Preset_Fullwidth
	extends WPDDL_Row_Type_Preset_Fullwidth{
	...
	public function htmlOpen( $markup, $args, $row = null, $renderer = null ) {
		...
		if( $args['mode'] === $this->id ) {
			...
			$args['container_class'] = 'container-fluid';
	...
	public function htmlClose( $output, $mode, $tag ) {
		...
		if( $mode = $this->id ) {
			...
			$output = '</' . $tag . '></div>'.$same_height_colums;
		}
