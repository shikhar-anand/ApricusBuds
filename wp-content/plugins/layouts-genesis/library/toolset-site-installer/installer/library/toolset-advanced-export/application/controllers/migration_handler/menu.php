...
class Menu implements IMigration_Handler {
    ...
    private function import_menus( $menus ) {
        ...
        $menu_id_map = array_reduce( $menus, function( $menu_id_map, $menu_data ) use( $results ) {
	        ...
	        if( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'tt_ajax' && $existing_term = get_term_by( 'slug', sanitize_title( $menu_name ), 'nav_menu' ) ) {
		        // toolset installer running and menu already exists (probably imported by WordPress right before)
		        if( $existing_term->count == 0 ) {
			        // empty menu, let's use it
			        $menu_id_map[ $old_menu_id ] = $existing_term->term_id;
			        $results->add( true, sprintf( __( 'Existing menu "%s" will be used.', 'toolset-advanced-export' ), sanitize_text_field( $menu_name ) ) );
			        return $menu_id_map;
		        }
	        }
